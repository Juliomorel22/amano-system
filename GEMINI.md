...
3. El solicitante elige una oferta, la acepta y transfiere el importe a un alias de la plataforma. Sube el comprobante.
4. El **Admin** verifica el pago, lo que libera y muestra la dirección exacta y teléfono entre el solicitante y el prestador.
5. El prestador realiza el trabajo; al finalizar, el solicitante confirma en la app.
6. La plataforma retiene una comisión (10%) y libera el saldo restante (90%) a favor del prestador. Se habilita calificación mutua.
...
- **`jobs`**: Entidad principal. Los pedidos de servicios, su estado (`open`, `accepted`, `paid`, `in_progress`, `completed`), cliente creador y categoría.
- **`offers`**: Las propuestas económicas y mensajes de los prestadores en relación a los trabajos abiertos (`job_id`).
- **`payments`**: Registro y tracking en storage del comprobante de transferencia y la validación por parte del Admin (`verified_at`, `verified_by`).
